...
export function App() {
  ...
  return <main className="app-shell">
    <header><strong>NOTERM</strong><select value={vaultId} onChange={e => setVaultId(e.target.value)}>{vaults.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}</select><span>{activeVault?.path}</span><button onClick={() => sync("pull")}>Pull</button><button onClick={() => sync("push")}>Push</button><button onClick={openFabric} disabled={!note}>Fabric</button><button onClick={openSettings}>Settings</button><button onClick={createCollection}>New collection</button><button onClick={create}>New note</button><button onClick={publish} disabled={!note}>Publish</button><button onClick={save} disabled={!note}>Save</button></header>
